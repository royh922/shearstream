...
//! \brief Problem generator for HD galaxy cluster based on M87. By Trung Ha
//! (UNT)
//!

/*
Virgo cluster setup, using similar initial condition as Guo, Stone, Kim, &
Quataert, 2023

This simplified Problem Generator includes: perturbation, gravity, radiative
cooling, cooling-based AMR only, Gaussian open jet profile with
accretion-dependent power only, inner/outer spherical boundaries, cold accretion
...
Real initialNumberDensityCGS,
    initialNumberDensityAstronomical;  // initial condition at x=1, used to
...
Real accretionUpdateFrequency,
    accretionMaxTemperature;  // update accretion rate (aka jet power) every n
                              // Myr, set maximum temperature of gas that is
                              // counted toward accretion
Real amrTimeOn;
...
bool restartFlag;
bool newGridFlag;
bool afterGridReconstruction = false;

Real newOuterBoundary;
int zoomInStep = 0;
int numberOfStepsToReachNewSink;  // how many time steps need to occur before
                                  // the inner region has shrunk to the new
                                  // inner radius
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
    ...
    haloRadius = pin->GetOrAddReal("problem", "r0_DM",
                                   6.e-2);  // halo scale radius in Mpc
    ...
    // Initial
    // conditions------------------------------------------------------------------------
    tabulatedNumberDensitiesAndPressures();  // generate a tabulated dataset of
                                             // initial densities and pressures
                                             // as a function of x
    densityFuncOfXArray = numberDensityFuncOfXArray;
    for (auto &it : densityFuncOfXArray) it *= conversionNtoRho;
    ...
    // Enroll Adaptive Mesh Refinement
    // conditions------------------------------------------------
    if (adaptive) {
    ...
    if (Globals::my_rank == 0) {
        ...
        if (restartFlag == true && newGridFlag == false) {
            std::cout << "----RESTART GALAXY-SCALE SIMULATION----"
                      << "\n";
            std::cout << "\n";
        } else if (restartFlag == true && newGridFlag == true) {
            std::cout << "----RESTART MESO-SCALE SIMULATION----"
                      << "\n";
            std::cout << "\n";
        } else {
            std::cout << "----NEW GALAXY-SCALE SIMULATION----"
                      << "\n";
            std::cout << "\n";
        }
        ...
        if (adaptive) {
            std::cout << "----ADAPTIVE MESH REFINEMENT----"
                      << "\n";
            // std::cout << "AMR is ON, up to an additional " <<
            // maxAdditionalAMRLevels << " refinement levels \n";
        ...
        if (outerRadius > codeLength) {
            std::cout << "WARNING: outer radius is outside the box, might "
                         "return nonphysical values"
                      << "\n";
        }
        std::cout << "\n";
    ...
    // "L_y", UserHistoryOperation::sum); EnrollUserHistoryOutput(5,
    // angularMomentumZ, "L_z", UserHistoryOperation::sum);
...
int RefinementCondition(MeshBlock *pmb) {
    ...
    // case certain conditions are met
        ...
        for (int j = pmb->js - 1; j <= pmb->je + 1; j++) {
            for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
                ...
                if (pmb->pmy_mesh->time < amrTimeOnCode) {
                    nestedLoopFlag = 1;
                    break;  // before AMR is turned on, only one step of the
                            // loop to record current refinement level is needed
                } else {
                    if (requiredSMRLevel >= maximumAllowedAMRLevel) {
                        nestedLoopFlag = 1;
                        break;  // if the SMR level is already high enough,
                                // ignore AMR
                    }
    ...
    if (requiredRefinementLevel > currentRefinementLevel)
        return 1;
    else if (requiredRefinementLevel < currentRefinementLevel)
        return -1;
    return 0;
}
...
void Mesh::UserWorkInLoop() {
    ...
    Real localInnerMass, localUpdatedColdMass,
        localUpdatedHotMass;  // value per MPI rank, to be summed over all ranks
                              // below
...
void innerRadialBoundary(MeshBlock *pmb, const AthenaArray<Real> &prim, AthenaArray<Real> &cons, int k, int j, int i,
    ...
    {                            // BAD FIX, CONSIDER WRITING MESHBLOCK::USERWORKINLOOP OR MEMORY ADDRESS
        ...
        // if (pmb->pmy_mesh->ncycle == 0 || (newGridFlag &&
        // pmb->pmy_mesh->ncycle == zoomInStep + 1)) { // at the start of the
        // sim, or when refined, record the base mass; at every time step after,
        // record the modified mass
        //     pmb->pmy_mesh->ruser_mesh_data[0](0) += primDensity *
        //     cellVolumeCode;
        // }
        ...
        for (int j = pmb->js; j <= pmb->je; ++j) {
            ...
            for (int i = pmb->is; i <= pmb->ie; ++i) {
                ...
                // OUTER RADIAL BOUNDARY
                    ...
                    outerRadialBoundary(pmb, prim, cons, k, j, i, z, y, x);
                }
                // END OUTER RADIAL BOUNDARIES
...
/*
Produce a table of normalized radial distances (dimensionlessX), densities, and
pressure
*/
...
void coolingSourceFunction(const Real dt, const AthenaArray<Real> &prim, AthenaArray<Real> &cons, int k, int j, int i) {
    ...
    if (conservedTemperatureCode <= temperatureFloor / codeTemperature) {
        coolingLossCode = 0.;  // no cooling if temperature at the start is
                               // lower than floor temperature
...
// static Real interpolate1D(Real inputX, std::vector<Real> tabulatedX,
// std::vector<Real> tabulatedY)
// {
//     std::vector<Real>::iterator upperIndexPointer;

//     upperIndexPointer = std::lower_bound(tabulatedX.begin(),
//     tabulatedX.end(), inputX); int upperIndexX = (upperIndexPointer -
//     tabulatedX.begin()); int lowerIndexX = upperIndexX - 1;
...
//     Real interpolatedY = (lowerBoundY * (upperBoundX - inputX) + upperBoundY
//     * (inputX - lowerBoundX)) / (upperBoundX - lowerBoundX);
...
static Real simpleInterpolate(Real inputX, std::vector<Real> tabulatedX, std::vector<Real> tabulatedY) {
    ...
    while (tabulatedX[upperIndexX] > inputX) {
        upperIndexX--;
    }

    Real lowerBoundX = tabulatedX[upperIndexX];
    Real upperBoundX = tabulatedX[upperIndexX + 1];
...
/*
Enforce a temperature and density floor if a cell has lower temperature/density
...
Real coldAccretionRate(MeshBlock *pmb, int iout) {  // unit is Msun yr^-1
    return (pmb->pmy_mesh->ruser_mesh_data[5](0)) * codeMass / codeTime / 1.e6;
}

Real hotAccretionRate(MeshBlock *pmb, int iout) {  // unit is Msun yr^-1
...
/*
CALCULATE TOTAL ANGULAR MOMENTUM OF THE SYSTEM IN EACH DIRECTION, NOT WRITTEN
YET
*/

Real angularMomentumX(MeshBlock *pmb,
                      int iout) {  // total angular momentum of the system
                                   // inside the jet region (exclude jet region)

    return 0.;
}

Real angularMomentumY(MeshBlock *pmb, int iout) { return 0.; }

Real angularMomentumZ(MeshBlock *pmb, int iout) { return 0.; }
// END USER HISTORY
// OUTPUT-------------------------------------------------------------------------